...
**M6 Enterprise core is also delivered:** org API keys (`Bearer pbk_…`, role-scoped, same RBAC matrix as humans), HMAC-signed webhooks with delivery log, the dispute flow (open after delivery → officer settles or reverses with refund), and INVITE_ONLY listings with invitations — all integration-tested (27 tests). Remaining M6: ERP field-mapping adapters, SSO, advanced reporting. See the go-live analysis for the remaining non-code steps (legal model, payment provider, corridor research).
